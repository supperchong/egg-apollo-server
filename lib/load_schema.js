...
const { graphqlLoader, resolverLoader, directiveLoader, schemaDirectiveLoader } = utils;
...
module.exports = app => {
  ...
  const isTs = process.env.EGG_TYPESCRIPT === 'true' && utils.extensions['.ts'];
  let filePaths = [];
  let fileNames = [];
  ...
  const loaders = [
    ...
    {
      match: isTs ? 'resolver.ts' : 'resolver.js',
      loader: resolverLoader,
      obj: resolverMap,
    },
    {
      match: isTs ? 'directive.ts' : 'directive.js',
      loader: directiveLoader,
      obj: directiveMap,
    },
    {
      match: isTs ? 'schemaDirective.ts' : 'schemaDirective.js',
